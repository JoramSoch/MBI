function PP = mbitest_nl(Y, x, X, V, deg, MBA, prior)
% _
% Testing for multivariate Bayesian inversion
% FORMAT PP = mbitest_nl(Y, x, X, V, deg, MBA, prior)
% 
...
% 
% FORMAT PP = mbitest_nl(Y, x, X, V, deg, MBA, prior) tests a multivariate
% Bayesian model for classification or regression of classes or targets x
...
if MBA.is_MBC
    ...
    mc = false;
    X2 = [ones(size(x)), zeros(size(Y2,1),deg)];
end;
X2 = [X2, X];
V2 = V;
...
for i = 1:n                     % loop over data points in the test set
    ...
    for j = find(prior.p)       % loop over label values (where prior is non-zero)
        ...
        if MBA.is_MBC           % classification -> categorical
            ...
            xij          = prior.x(j);
            if mc, xij   = xij - mean(x); end;
            for m = 1:deg, x2ij(m+1) = xij^m; end;
