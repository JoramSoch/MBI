function MBI = ML_MBI_nl(Y, x, X, V, CV, deg, type, prior)
% _
% Cross-Validated Multivariate Bayesian Inversion
% FORMAT MBI = ML_MBI_nl(Y, x, X, V, CV, deg, type, prior)
% 
...
%     deg   - an integer indicating the degree of polynomial expansion
...
%                      o deg   - an integer, the polynomial expansion order
...
% 
% FORMAT MBI = ML_MBI_nl(Y, x, X, V, CV, deg, type, prior) splits measured
% signals Y, classes/targets x and covariate values X into cross-validation
% folds according to CV and then performs multivariate Bayesian inversion
...
if nargin < 6 || isempty(deg), deg = 1;                   end;
...
p = strcmp(type,'MBC')*C + strcmp(type,'MBR')*(1+deg) + size(X,2);
...
for g = 1:k
    ...
    MBA1 = mbitrain_nl(Y1, x1, X1, V1, deg, type);
    
    % test data: B/T are known, infer on X
    PP(i2,:) = mbitest_nl(Y2, x2, X2, V2, deg, MBA1, prior);
